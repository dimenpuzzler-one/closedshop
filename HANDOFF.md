...
- 현재 `HalbuInfo=00`이라 일시불만 요청합니다. 공식 매뉴얼은 `HalbuInfo`를 필수 문자열로 정의하고 `00`은 일시불, `02`~`12`는 할부개월로 설명합니다. `PAYDATAKR_HALBU_INFO`로 이 값을 검증해 전달할 수 있게 했습니다. 매뉴얼에는 별도 가맹점 승인 절차가 적혀 있지 않으므로, 원하는 개월 정책을 정한 뒤 값을 설정하면 됩니다. `02`~`12`가 결제창에서 고객 선택 목록을 여는지 또는 선택한 개월을 고정하는지는 결제창 동작 또는 KPD 답변으로 확인해야 합니다. 확인 전에는 운영값을 `00`으로 유지합니다.
