...
> 마지막 갱신: **2026-09-01 (Asia/Seoul)** — 배송지/주소검색/선물 배송 UX 커밋 `9d84c83` 이후 변경 포함
...
- 폐쇄몰: 비로그인/비귀속 방문자에게는 **가격만 가립니다**(상품 자체는 보입니다). 가격은 렌더링에서 숨기는 게 아니라 **payload에서 제거**합니다(`stripPrices()`).
...
- 선물 주문은 받는 분과 보내는 사람/연락처를 분리해 주문 스냅샷에 보관하고, 결제 고객 정보에는 보내는 사람을 우선 사용합니다.
...
2. **회원가입 진입점 없음** — 로그아웃 상태에서 가입 화면으로 갈 링크가 없습니다
3. **가입 순서 미합의** — 대표님: "구글 가입 먼저 → 추천인 코드" / 형님: "추천인 코드 없으면 가입 자체가 안 되는 게 맞다". 결정 필요
4. 리뷰 기능 — 보류(회원제라 의미 있는지 미결)
5. 가입 전 가격 노출 여부 — 보류

### 운영/법무 미결

6. 3PL 정식 상호를 개인정보 위탁 표에 기입
7. PG사(코페이) 상호를 위탁 표에 기입
8. 개인정보 **국외이전** 고지 — Vercel 실제 처리 지역 확인 필요(개인정보 보호법 제28조의8)
9. 코페이 **취소/환불 API 규격** 요청 — 현재 환불은 관리자 화면에서 상태만 바꿉니다. 실제 PG 취소 연동이 없습니다
10. 이메일 인증(Confirm email) 설정 및 SMTP 확인
11. `tester@dealkey.co.kr` 오픈 전 삭제

### 기술 부채 (2026-09-01 코드 점검에서 확인)

12. `apps/admin/app/api/products/route.ts`의 **multipart 분기가 죽은 코드**입니다(~130줄). 등록 폼은 항상 JSON으로 보내고 이미지는 별도 경로로 올립니다. 이 죽은 경로에 4MB 제한이 남아 있어 실제 20MB 제한과 어긋나 보입니다
13. `order-service.ts`(521줄)와 `cart-pricing.ts`가 **상품/옵션/재고 조회 로직을 각각 따로** 갖고 있습니다. 한쪽만 고치면 장바구니 미리보기와 실제 결제 금액이 갈라집니다
14. `OrderStatus`/`CommissionStatus` 값 목록이 `types` / `validation` / `db` **세 곳에 따로** 박혀 있습니다
15. `packages/validation`, `packages/payment`의 `@closed-commerce/types` 의존성이 선언만 되고 안 쓰입니다
16. 어드민 폼 3개에 `readResponse`/에러 처리가 거의 같은 모양으로 복붙돼 있습니다

### 데이터 정리

17. 육포 420g 두 종(`...420g`, `...420g+쇼핑백`)은 `status='paused'`, 이미지 0장입니다. 판매하려면 이미지 등록 + active 전환 필요
18. **과대 이미지 4장** — 업로드 한도(20MB) 안이라 통과했지만 실제로는 너무 큽니다. 특히 대표 이미지는 목록 카드에도 쓰여서 목록 로딩을 직접 끌어내립니다.
