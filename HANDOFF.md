...
- 현재까지는 `HalbuInfo=00`이라 일시불만 요청했지만, 이제 주문서에서 결제 예정 금액이 5만원 이상이면 `00` 또는 `02`~`12`개월을 선택할 수 있습니다. 5만원 미만 주문은 서버가 `00`으로 강제합니다. 공식 매뉴얼은 `HalbuInfo`를 필수 문자열로 정의하고 `00`은 일시불, `02`~`12`는 할부개월로 설명합니다. `PAYDATAKR_HALBU_INFO`는 주문서 선택값이 없을 때 사용할 기본 요청값으로 검증합니다.
