-- admin_update_product를 청약철회 제한 안내까지 수정할 수 있게 넓힌다.
-- 기존 함수를 그대로 복사하고 두 곳만 넓혔다: UPDATE 절과 반환 객체.
-- 처음부터 다시 쓰면 shortDescription·reservedStock 같은 항목이 조용히 빠진다.
...
begin
  ...
  update public.products
     set name = case when p_patch ? 'name' then p_patch->>'name' else name end,
         ...
         -- 전자상거래법 제17조 제2항 단서: 이 문구를 표시하지 않으면 청약철회 제한을 주장할 수 없다.
