-- 상품별 배송 정책을 저장한다.
-- shipping_fee는 묶음 1회당 금액(0이면 무료배송),
-- shipping_bundle_quantity는 한 번에 묶을 수 있는 수량이다.
alter table public.products
  add column if not exists shipping_bundle_quantity integer not null default 1;

alter table public.products
  drop constraint if exists products_shipping_bundle_quantity_check;

alter table public.products
  add constraint products_shipping_bundle_quantity_check
  check (shipping_bundle_quantity >= 1 and shipping_bundle_quantity <= 1000);

comment on column public.products.shipping_bundle_quantity is
  '상품별 배송비 1회에 묶을 수 있는 수량';

-- 기존 상품은 지금까지 쓰던 운영 기본값을 이어받는다.
update public.products
   set shipping_bundle_quantity = coalesce(
     (select shipping_carton_quantity from public.store_settings where id = 1),
     5
   );

-- 상품 수정 RPC에도 상품별 묶음 수량을 포함한다.
...
begin
  ...
  update public.products
     set name = case when p_patch ? 'name' then p_patch->>'name' else name end,
         ...
         shipping_bundle_quantity = case when p_patch ? 'shippingBundleQuantity' then (p_patch->>'shippingBundleQuantity')::integer else shipping_bundle_quantity end,
  ...
  select jsonb_build_object(
    ...
    'shippingBundleQuantity', p.shipping_bundle_quantity,
