...
The web checkout POSTs the documented authentication form fields directly to `/kpdWebPayment/KpdCredit` in a named payment window (`popuptype=popup`). The payment popup first displays the installment dropdown; after the customer chooses a term, the same window submits the KPD authentication form. If the browser blocks the popup, the checkout falls back to the current window. No browser SDK, widget token, or checkout URL environment variable is required. `PAYDATAKR_API_BASE_URL` defaults to `https://api.paydatakr.com`. The checkout enables `00` (one-time) and `02` through `12` (installment months) in the payment popup when the server-calculated amount is at least KRW 50,000; amounts below that threshold are forced to `00`. `PAYDATAKR_HALBU_INFO` remains an optional server default for callers that do not provide a checkout selection. The PayDataKR manual does not describe a separate approval step for this field. The result and cancellation targets are API routes that accept cross-origin form POSTs and redirect with 303; do not point them directly at Next.js pages. Once the values are entered in the appropriate Vercel project environments and redeployed, the PayDataKR flow is enabled without another code change.
