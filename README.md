...
| 방문자                    | 상품 목록·상세 | 가격       | 장바구니·주문 |
| ------------------------- | -------------- | ---------- | ------------- |
| 비회원                    | 보임           | 비노출     | 불가          |
| 회원 (추천 코드 귀속 없음) | 보임           | 비노출     | 불가          |
| 회원 (추천 코드 귀속)      | 보임           | 특판가     | 가능          |
| 위 + 프로모션 코드         | 보임           | 추가 할인  | 가능          |
...
- 가격은 화면에서 가리는 것이 아니라 **서버 페이로드에서 제거**합니다(`stripPrices`). HTML 소스에 숫자가 남으면 가린 것이 아닙니다.
- `POST /api/cart/quote`도 자격 없는 회원에게는 견적을 반환하지 않습니다. 주문 생성은 추천 귀속이 없으면 `403`입니다.

주의: 비회원 카탈로그는 세션 클라이언트로 읽을 수 없어(RLS가 0행) service role로 읽습니다. 따라서 이 경로의 가격 차단은 RLS가 아니라 애플리케이션 코드가 담당합니다. `catalog-data.ts`를 수정할 때 `stripPrices` 호출이 빠지지 않는지 확인해야 합니다.
...
관리자 `/settings`에서 개발자 없이 바꿀 수 있는 값입니다.

| 항목      | 내용                                                             |
| --------- | ---------------------------------------------------------------- |
| 배송비    | 묶음 수량, 묶음당 요금, 무료배송 기준액(선택), 배송 마감 시간    |
| 카테고리  | 추가·삭제. 상품이 붙어 있는 카테고리는 삭제를 거부합니다         |
| 홈 화면   | 메인 문구, 메인 설명, 유튜브 링크, 배너 이미지                   |
...
- 배너 이미지는 상품 사진과 같은 서명 업로드 경로를 사용합니다(`banners/` 접두사, 20MB 제한). Vercel 본문 한도를 거치지 않습니다.
...
- 비로그인 방문자에게는 상품과 상세 내용은 보여주되 가격은 서버 응답에서 제거합니다. 추천 코드로 가입·승인된 회원에게만 회원가와 주문 기능을 제공합니다.
...
- 비회원 카탈로그 경로의 가격 차단은 RLS가 아니라 애플리케이션 코드(`stripPrices`)에 의존합니다.
