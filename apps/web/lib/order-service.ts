...
export interface PersistedOrderResult {
  ...
  commissionPreview: Pick<
    CommissionSnapshot,
    'depth' | 'beneficiaryName' | 'commissionAmount' | 'status'
  >[];
}
...
async function loadPromotion(
  ...
  const { data: code, error: codeError } = await client
    ...
    .select(
      'id, code, status, starts_at, expires_at, total_usage_limit, per_member_usage_limit, usage_count',
    )
    .eq('code', normalized)
    .eq('status', 'active')
    .maybeSingle();
  ...
  if (
    ...
    fail(400, '현재 사용할 수 없는 Promotion Code입니다.');
  if (
    code.total_usage_limit !== null &&
    code.usage_count >= code.total_usage_limit
  )
    fail(400, 'Promotion Code 사용 한도가 끝났습니다.');
  if (code.per_member_usage_limit !== null) {
    ...
    if ((count ?? 0) >= code.per_member_usage_limit)
      fail(400, '회원별 Promotion Code 사용 한도를 초과했습니다.');
  }
  const { data: rule, error: ruleError } = await client
    .from('promotion_rules')
    .select(
      'product_ids, referral_code_ids, minimum_order_amount, minimum_quantity, discount_rate, discount_amount',
    )
    .eq('promotion_code_id', code.id)
    .maybeSingle();
  if (ruleError || !rule) fail(400, 'Promotion 조건을 찾을 수 없습니다.');
  if (
    rule.referral_code_ids.length > 0 &&
    !rule.referral_code_ids.includes(referralCodeId)
  )
    fail(400, '이 Referral Code에는 적용할 수 없는 Promotion입니다.');
  return {
    id: code.id,
    code: code.code,
    status: code.status,
...
async function loadCatalog(
  client: AppSupabaseClient,
  input: CreateOrderInput,
): Promise<{ lines: CatalogLine[]; products: Product[] }> {
  ...
  const [
    ...
    client
      .from('products')
      .select(
        'id, slug, name, category, short_description, description, base_price, supply_cost, shipping_fee, shipping_bundle_quantity, visibility, status, created_at',
      )
      .in('id', productIds)
      .eq('status', 'active')
  ...
  if (productError || optionError || inventoryError)
    fail(500, '상품 정보를 불러오지 못했습니다.');
  if ((products?.length ?? 0) !== productIds.length) {
    const found = new Set((products ?? []).map((product) => product.id));
    const missing = productIds.filter((id) => !found.has(id));
    fail(
      400,
      `판매 중인 상품이 아니거나 상품을 찾을 수 없습니다. (${missing.length}건)`,
    );
  }
...
async function loadReferral(
  ...
  const { data: relationship, error: relationshipError } = await client
    ...
    .select(
      'id, referred_user_id, referrer_user_id, referral_code_id, source, campaign_id, created_at',
    )
    .eq('referred_user_id', buyerUserId)
    .maybeSingle();
  if (relationshipError || !relationship)
    fail(403, '회원의 최초 Referral 귀속을 확인할 수 없습니다.');
  const { data: referral, error: referralError } = await client
    .from('referral_codes')
    .select(
      'id, code, owner_user_id, campaign_id, status, starts_at, expires_at, created_at',
    )
    .eq('id', relationship.referral_code_id)
    .eq('status', 'active')
    .maybeSingle();
  if (referralError || !referral)
    fail(403, '최초 Referral Code가 더 이상 유효하지 않습니다.');
  if (referralCode && referral.code !== referralCode.trim().toUpperCase())
    fail(400, '최초 가입 Referral Code와 다른 코드로 주문할 수 없습니다.');
  if (referral.owner_user_id !== relationship.referrer_user_id)
    fail(500, 'Referral 귀속 데이터가 일치하지 않습니다.');
  ...
  const beneficiaryIds = [
    ...
    ...(parentRelationship ? [parentRelationship.referrer_user_id] : []),
  ];
  ...
  lookup.set(buyerUserId, {
    userId: relationship.referrer_user_id,
    name:
      names.get(relationship.referrer_user_id) ?? relationship.referrer_user_id,
  });
  if (parentRelationship)
    lookup.set(relationship.referrer_user_id, {
      userId: parentRelationship.referrer_user_id,
      name:
        names.get(parentRelationship.referrer_user_id) ??
        parentRelationship.referrer_user_id,
    });
...
async function loadShippingPolicy(
  ...
  const { data, error } = await client
    .from('store_settings')
    .select(
      'shipping_fee_per_carton, shipping_carton_quantity, free_shipping_threshold',
    )
    .eq('id', 1)
    .maybeSingle();
  ...
  return {
    cartonQuantity:
      data.shipping_carton_quantity ?? DEFAULT_SHIPPING_POLICY.cartonQuantity,
    feePerCarton:
      data.shipping_fee_per_carton ?? DEFAULT_SHIPPING_POLICY.feePerCarton,
 ...
 *
 * 예전에는 이 함수 하나가 주문 생성부터 결제 승인까지 다 했다. 한국결제데이터 인증결제는
...
export async function prepareOrder(
  ...
  if (sweepError)
    logServerError('order.prepare', requestId, sweepError, {
      stage: 'expire_stale',
    });
  ...
  if (
    promotion &&
    (promotion.rule.discountRate !== undefined ||
      promotion.rule.discountAmount !== undefined) &&
  ...
  const orderNumber = randomBytes(8)
    .readBigUInt64BE()
    .toString(36)
    .toUpperCase()
    .padStart(13, '0');
  ...
  try {
    ...
    for (const line of lines) {
      const { data: reserved, error: reserveError } = await client.rpc(
        'reserve_inventory',
        { p_product_id: line.productId, p_quantity: line.quantity },
      );
      if (reserveError || !reserved)
        fail(409, `${line.productName} 재고가 방금 소진되었습니다.`);
    ...
    logServerError('order.prepare', requestId, error, {
      stage: 'rollback',
      orderId,
      reservedCount: reservedLines.length,
    });
    ...
    if (orderCreated) {
      ...
      if (cancelError)
        logServerError('order.prepare', requestId, cancelError, {
          stage: 'order_cancel',
          orderId,
        });
    }
...
async function releaseReservations(
  ...
  released.forEach((result, index) => {
    if (result.error)
      logServerError('order.compensate', requestId, result.error, {
...
export async function finalizePayDataKrOrder(
  ...
  const orderNumber =
    typeof result.trackId === 'string' ? result.trackId.trim() : '';
  ...
  const { data: order, error: orderError } = await client
    .from('orders')
    .select(
      'id, order_number, status, buyer_user_id, referrer_user_id, referral_code, promotion_code, gross_amount, discount_amount, shipping_amount, paid_amount, commissionable_amount, paid_at',
    )
  ...
  if (order.status !== 'payment_pending') {
    if (
      ...
      return {
        ...
        payment: {
          paymentId: payment?.provider_payment_id ?? transactionId,
          orderId: order.id,
  ...
  try {
    ...
    verifyPayDataKrLookup(lookupResult, {
      trackId: order.order_number,
      transactionId,
      amount: order.paid_amount,
    });
  } catch (error) {
    logServerError('order.finalize', requestId, error, {
      stage: 'lookup',
      orderId: order.id,
      transactionId,
      source: input.source,
    ...
    fail(
      502,
      '결제 결과를 확인하지 못했습니다. 잠시 후 주문 내역을 확인해 주세요.',
    );
  }
  ...
  if (gatewayAmount === undefined || gatewayAmount !== order.paid_amount) {
    logServerError(
      'order.finalize',
      requestId,
      new Error('approved amount mismatch'),
      {
        orderId: order.id,
  ...
  // webhook과 return이 동시에 도착해도 주문당 결제 행을 하나만 만들 수 있다.
  ...
  if (claimError) {
    if (claimError.code === '23505') {
      logServerEvent('order.finalize', requestId, {
        stage: 'duplicate_notification',
        orderId: order.id,
        source: input.source,
      });
      fail(
        409,
        '결제 결과를 처리 중입니다. 잠시 후 주문 내역을 확인해 주세요.',
      );
    }
    logServerError('order.finalize', requestId, claimError, {
      stage: 'payment_claim',
      orderId: order.id,
    });
    fail(500, '결제 결과를 저장하지 못했습니다.');
  }
  ...
  const { error: paymentError } = await client
    ...
    .eq('status', 'pending');
  if (paymentError) {
    logServerError('order.finalize', requestId, paymentError, {
    ...
    fail(
      500,
      '결제는 완료됐지만 기록에 실패했습니다. 고객센터로 문의해 주세요.',
    );
  }
  ...
  const { lookup } = await loadReferral(
    client,
    order.buyer_user_id,
    order.referral_code ?? undefined,
  );
  ...
  if (commissions.commissions.length > 0) {
    ...
    if (commissionError)
      logServerError('order.finalize', requestId, commissionError, {
        stage: 'commission_insert',
        orderId: order.id,
      });
  }

  if (order.promotion_code) {
    const { data: promotionRow } = await client
      .from('promotion_codes')
      .select('id')
      .eq('code', order.promotion_code)
      .maybeSingle();
    if (promotionRow) {
      const { error: redemptionError } = await client.rpc(
      ...
      if (redemptionError)
        logServerError('order.finalize', requestId, redemptionError, {
          stage: 'promotion_redeem',
          orderId: order.id,
        });
    }
  }
  ...
  return {
    ...
    commissionPreview: commissions.commissions.map(
      ({ depth, beneficiaryName, commissionAmount, status }) => ({
        depth,
        beneficiaryName,
        commissionAmount,
