...
export async function finalizePayDataKrOrder(
  ...
  const { error: paymentError } = await client
    ...
    .eq('status', 'pending');
  if (paymentError) {
    logServerError('order.finalize', requestId, paymentError, { stage: 'payment_update', orderId: order.id, transactionId });
  ...
  logServerEvent('order.finalize', requestId, { stage: 'paid', source: input.source, orderId: order.id, orderNumber: order.order_number, transactionId, paidAmount: order.paid_amount });
  await client.from('analytics_events').insert({
    user_id: order.buyer_user_id,
    event_name: 'order_paid',
    referral_code: order.referral_code,
    referrer_user_id: order.referrer_user_id,
    properties: { order_id: order.id, amount: order.paid_amount, transaction_id: transactionId },
  });

  return {
    orderId: order.id,
    orderNumber: order.order_number,
    payment: { paymentId: transactionId, orderId: order.id, amount: gatewayAmount, status: 'paid', paidAt },
    totals: {
      grossAmount: order.gross_amount,
      discountAmount: order.discount_amount,
      shippingAmount: order.shipping_amount,
