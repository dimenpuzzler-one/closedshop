...
function mapProduct(row: { id: string; slug: string; name: string; short_description: string; description: string; base_price: number; shipping_fee: number; visibility: Product['visibility']; status: Product['status'] }, options: { id: string; name: string; value: string; price: number; stock: number }[]): Product {
  return { id: row.id, slug: row.slug, name: row.name, shortDescription: row.short_description, description: row.description, weight: options[0]?.value ?? '', price: row.base_price, shippingFee: row.shipping_fee, visibility: row.visibility, status: row.status, imageUrl: '', options, tags: [] };
}
...
export async function loadVisibleCatalog(referralCode?: string): Promise<{ products: Product[]; validReferralCode?: string; authenticated: boolean }> {
  ...
  const products = rows.map((row) => {
    ...
    return mapProduct(row, rowOptions);
  });
