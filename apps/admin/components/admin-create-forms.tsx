...
function ImagePicker({ label, name, multiple = false, hint }: { label: string; name: string; multiple?: boolean; hint: string }) {
  ...
  function handleChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    setPreviews(Array.from(event.currentTarget.files ?? []).map((file) => URL.createObjectURL(file)));
  }
  useEffect(() => () => previews.forEach((preview) => URL.revokeObjectURL(preview)), [previews]);
  return <div className="field"><span className="field-label">{label}</span><input className="input" type="file" name={name} accept="image/jpeg,image/png,image/webp" multiple={multiple} onChange={handleChange} /><span className="field-hint">{hint}</span>{previews.length ? <div className="image-preview-grid" aria-label="이미지 미리보기">{previews.map((preview, index) => <div className="image-preview" key={preview}><Image src={preview} alt={`선택 이미지 ${index + 1}`} width={160} height={100} unoptimized /></div>)}</div> : null}</div>;
}

export function ProductCreateForm() {
  const form = useCreate('/api/products', { multipart: true });
  return <details className="card admin-section"><summary className="button button-secondary">상품 등록 열기</summary><form className="stack" onSubmit={form.submit} encType="multipart/form-data"><p className="field-hint">옵션은 고객이 선택하는 구성·중량입니다. 단일 구성 상품이면 기본값을 그대로 쓰고 옵션가는 비워두세요.</p><div className="form-grid"><label className="field"><span className="field-label">Slug</span><input className="input" name="slug" placeholder="premium-pear-500g" required /><span className="field-hint">영문 소문자, 숫자, 하이픈만 사용합니다.</span></label><label className="field"><span className="field-label">상품명</span><input className="input" name="name" required /></label><label className="field"><span className="field-label">제품 카테고리</span><input className="input" name="category" defaultValue="기타" maxLength={80} required /><span className="field-hint">고객몰에서 상품을 분류할 이름입니다.</span></label><label className="field"><span className="field-label">기본가</span><input className="input" type="number" min="0" name="basePrice" required /><span className="field-hint">옵션가를 비워두면 이 금액이 판매가가 됩니다.</span></label><label className="field"><span className="field-label">공급가(선택)</span><input className="input" type="number" min="0" name="supplyCost" /></label><label className="field"><span className="field-label">배송비</span><input className="input" type="number" min="0" name="shippingFee" defaultValue="0" required /></label><label className="field"><span className="field-label">노출 대상</span><select className="select" name="visibility" defaultValue="referral"><option value="referral">추천 회원 전용</option><option value="member">회원 전용</option><option value="public">공개</option><option value="hidden">비공개</option></select></label><label className="field"><span className="field-label">판매 상태</span><select className="select" name="status" defaultValue="active"><option value="active">즉시 판매</option><option value="draft">초안</option><option value="paused">판매 중지</option></select></label><label className="field"><span className="field-label">옵션명</span><input className="input" name="optionName" defaultValue="구성" required /></label><label className="field"><span className="field-label">옵션값</span><input className="input" name="optionValue" placeholder="예: 300g / 기본 구성" required /></label><label className="field"><span className="field-label">옵션가(선택)</span><input className="input" type="number" min="0" name="optionPrice" /><span className="field-hint">옵션별 최종 판매가입니다. 비워두면 기본가를 사용합니다.</span></label><label className="field"><span className="field-label">초기재고</span><input className="input" type="number" min="0" name="stock" required /></label></div><label className="field"><span className="field-label">짧은 소개</span><input className="input" name="shortDescription" maxLength={300} placeholder="목록에 표시할 한 줄 소개" /></label><label className="field"><span className="field-label">상세페이지 설명</span><textarea className="textarea" name="description" maxLength={4000} placeholder="고객이 상세 페이지에서 볼 상품 설명" /></label><div className="form-grid"><ImagePicker label="썸네일 이미지(선택, 1장)" name="thumbnail" hint="JPG, PNG, WEBP / 5MB 이하" /><ImagePicker label="상세페이지 이미지(선택, 여러 장)" name="detailImages" multiple hint="최대 8장, 각 5MB 이하" /></div><button className="button button-primary">상품 등록</button>{form.error ? <p className="admin-note">{form.error}</p> : null}{form.message ? <p className="admin-note">{form.message}</p> : null}</form></details>;
}

export function ShippingSettingsForm({ defaultValue }: { defaultValue: string }) {
  const form = useCreate('/api/settings');
  return <details className="card admin-section"><summary className="button button-secondary">배송 마감 설정 열기</summary><form className="stack" onSubmit={form.submit}><label className="field"><span className="field-label">배송 마감 시간</span><input className="input" type="time" name="shippingCutoffTime" defaultValue={defaultValue} required /><span className="field-hint">기본값은 14:00이며, 이 시간은 고객몰에도 안내됩니다.</span></label><button className="button button-primary">배송 설정 저장</button>{form.error ? <p className="admin-note">{form.error}</p> : null}{form.message ? <p className="admin-note">{form.message}</p> : null}</form></details>;
}
