...
export async function POST(request: Request) {
  ...
  const { data: product, error: productError } = await context.client.from('products').insert({ slug: parsed.data.slug, name: parsed.data.name, short_description: parsed.data.shortDescription, description: parsed.data.description, base_price: parsed.data.basePrice, shipping_fee: parsed.data.shippingFee, visibility: parsed.data.visibility, status: parsed.data.status }).select('id').single();
  if (productError || !product) return NextResponse.json({ error: '상품을 저장하지 못했습니다.' }, { status: 500 });
  ...
  if (optionError || inventoryError) return NextResponse.json({ error: '상품 옵션 또는 재고를 저장하지 못했습니다.' }, { status: 500 });
